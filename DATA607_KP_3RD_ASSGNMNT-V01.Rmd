...
This is a week 3 assignment, working with strings. The assignment has three different sections each will be presented below separately.
...
After loading the data, we use code of strings like those have been shared to filter for "DATA" and "SCIENCE" as well s the ones that contain both.

```{r find_string, echo=TRUE}

#using basic function of R
#search for data
...
#search for thsoe with either DATA or STATISTICS
print("Here is the list of the majors that have either DATA or STATISTICS")
Contains_DATA_STATISTICS<- grep(pattern='DATA|STATISTICS', Major_data$Major,value = TRUE, ignore.case = TRUE)
print(Contains_DATA_STATISTICS)

#since grep only check for the frist string, we need to create a loop to go thru the entire length of the majors 
...
Contains_DATA_STATISTICSE<- dplyr::filter(Major_data, grepl('STATISTICS|DATA',Major))
#Contains_DATA_STATISTICS<- dplyr::filter(Major_data, grepl('STATISTICS&DATA',Major))


result_matrix <- data.frame(Sel_Majors=character())


#pmatch('DATA', Major_data$Major, duplicates.ok = FALSE)
```
...
#To make this excersise realsitic I entered all thoe in a text file and them will
...
| "(.)(.)\\\\2\\\\1"                                        | matches any sequence of four characters where the 1st and 4th are the same, and the 2nd and 3rd are the                                                                                                                                     |
| same but can be different or the same as the 1st and 4th. |                                                                                                                                                                                                                                             |
